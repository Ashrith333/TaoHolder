...
export function Seg<T extends string>({ options, value, onChange, label, full }: { options: Option<T>[]; value: T; onChange: (v: T) => void; label: string; full?: boolean }) {
  return (
    <div role="tablist" aria-label={label} className={`inline-flex rounded-[14px] bg-s2 p-1 ${full ? "w-full" : ""}`}>
      {options.slice(0, 5).map((o) => (
        <button
          ...
          className={`min-h-10 flex-1 rounded-[10px] px-3 text-[13px] font-semibold transition-colors ${
